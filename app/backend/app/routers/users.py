...
from app.core.auth import get_current_user, get_google_payload
...
async def check_name(
    ...
    _payload: dict = Depends(get_google_payload),
...
async def register(
    body: UserRegisterIn,
    payload: dict = Depends(get_google_payload),
    session: AsyncSession = Depends(get_session),
) -> User:
    ...
    user = User(google_id=google_id, name=body.name)
    session.add(user)
